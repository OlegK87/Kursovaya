...
def main():
    class VKDownloader:
        ...
        def get_photos(self, offset=0, count=20):
    ...
    vk_token = input("Введите Ваш токен в VK: ")
    downloader = VKDownloader(vk_token)
    downloader.get_all_photos()

    ya_token = str(input('Введите Ваш токен на Яндекс.Диск: '))
    uploader = YaUploader(ya_token)
    folder_name = str(input('Введите имя папки на Яндекс.Диск для записи фотографий: '))
    ...
    for photo in photos_list:
        ...
        count += 1
        print(f'Всего загружено на Яндекс.Диск: {count} фотографий')
